...
def plot_scatter_diagram(which_fig, x, y, x_label = 'x', y_label = 'y', title = 'title', style_list = None):
	...
	'''
	styles = ['k', 'g', 'r', 'c', 'm', 'y', 'b', '#9400D3','#C0FF3E','#FFEBCD','#0000FF','#8A2BE2','#A52A2A','#DEB887','#5F9EA0','#7FFF00']
	assert len(x) == len(y)
	...
	if style_list is None:
		#plt.plot(x, y, color=styles[0],  marker='.')
		plt.scatter(x,y,edgecolors=styles[0])
	else:
